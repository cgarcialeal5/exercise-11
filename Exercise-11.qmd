...
In order to add a count of individuals in the sample of each order, I ran this code before creating the plots:
...
p_maxlife <- ggplot(d4, aes(x = order, y = relLife))+
  ...
  geom_text(data = Orders, aes(x = order, y = y_min1 - 0.5, label = paste0("n=", n)), color = "blue")
...
```{r}
y_min2 <- min(d4$relNewbornMass)
p_NewbornMass <- ggplot(d4, aes(x = order, y = relNewbornMass))+
  geom_violin() +
  stat_summary(fun = mean, geom = "point", color = "red") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Order", y = "Real Newborn Mass (g)") +
  geom_text(data = Orders, aes(x = order, y = y_min2 - 0.5, label = paste0("n=", n)), color = "blue")
print(p_NewbornMass)
```
...
```{r}
y_min3 <- min(d4$relWeanMass)
p_WeaningMass <- ggplot(d4, aes(x = order, y = relWeanMass))+
  geom_violin() +
  stat_summary(fun = mean, geom = "point", color = "red") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Order", y = "Real Weaning Mass (g)") +
  geom_text(data = Orders, aes(x = order, y = y_min3 - 0.5, label = paste0("n=", n)), color = "blue")
print(p_WeaningMass)
```
